...
async def build_execute_recommendation_query(name_ids: List[str], plant_id: str) -> Tuple[List[RecommendationTargetEntitySchema], List[RecommendationPairElemSchema], List[RecommendationPairElemSchema]]:
    """
    Build and execute the recommendation query
    """
    try:
        logger.info(f"Building recommendation query for name_ids: {name_ids}, plant_id: {plant_id}")
        
        query = RECOMMENDATION_TEMPLATE.format(name_ids=name_ids)
        logger.info(f"Generated Neo4j query: {query[:200]}...")
        ...
        logger.info(f"Neo4j query returned {len(res)} results")
        ...
        if calc_engine_result and len(calc_engine_result) == 3:
            ...
            return calc_engine_request.targets, [], []
        ############################################
    except Exception as e:
        logger.error(f"Error in build_execute_recommendation_query: {e}")
        print(f"❌ build_execute_recommendation_query failed: {e}")
        # Return empty results with just the targets
        targets = [RecommendationTargetEntitySchema(name_id=name_id) for name_id in name_ids]
        return targets, [], []
...
async def execute_neo4j_query(query:str,plant_id:str)->List[dict]:
    ...
    try:
        ...
        logger.error(f"Error executing Neo4j query for plant {plant_id}: {e}")
