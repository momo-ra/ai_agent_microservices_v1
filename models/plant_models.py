...
class Artifacts(PlantBase, BaseModel):
    """Artifacts - Plant Database, Plant-wide AI Access"""
    __tablename__ = 'artifacts'
    
    session_id = Column(String, ForeignKey('chat_sessions.session_id'), nullable=False)
    user_id = Column(Integer, nullable=False)  # References users.id from central DB (no FK constraint)
    title = Column(String(255), nullable=False)
    artifact_type = Column(String(50), nullable=False, default='general')
    content = Column(Text, nullable=False)
    artifact_metadata = Column(JSON, nullable=True)
    is_active = Column(Boolean, default=True)
    message_id = Column(Integer, nullable=True)  # References chat_messages.id (no FK constraint)
